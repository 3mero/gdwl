...
export function ScheduleManager({ 
  ...
  return (
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md flex flex-col p-3.5 sm:p-5 overflow-hidden rounded-l-2xl sm:rounded-l-3xl border-none shadow-2xl bg-background" dir="rtl">
        {currentView === 'notifications' ? (
          <>
            <SheetHeader className="text-right px-1 pt-1 pb-3 pe-12 border-b space-y-1">
              <Button 
                variant="ghost" 
                size="sm" 
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-1 py-3 custom-scrollbar">
          ...
          <>
            <SheetHeader className="text-right px-1 pt-1 pb-3 pe-12 space-y-1">
              <SheetTitle className="text-xl font-bold tracking-tight">الإعدادات</SheetTitle>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-2 py-3 space-y-4 custom-scrollbar">
              <div className="space-y-3.5">
                ...
                <div className="p-3.5 rounded-xl border border-border/70 bg-accent/25 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-right">
                  ...
                  <Button
                    ...
                    className="h-8 text-xs gap-1.5 w-full sm:w-auto shrink-0 border-primary/30 text-primary hover:bg-primary/10 font-bold justify-center"
                  >
                ...
                <div 
                  ...
                  className="p-3.5 rounded-xl border border-blue-500/25 bg-blue-500/5 hover:bg-blue-500/10 transition-colors flex items-center justify-between gap-2 text-right cursor-pointer group"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="h-9 w-9 rounded-xl bg-blue-500/15 flex items-center justify-center text-blue-500 shrink-0">
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-xs sm:text-sm">إدارة الجداول</h3>
                    <Button onClick={() => setIsCreateDialogOpen(true)} size="sm" variant="outline" className="h-7 text-xs gap-1">
                      <Plus className="h-3.5 w-3.5" /> جدول جديد
                    </Button>
                  </div>
                  <div className="space-y-2">
                    {schedules.map((schedule) => (
                      <div key={schedule.id} className="flex items-center justify-between gap-3 rounded-xl border border-border/70 bg-card/60 p-2.5 px-3.5 hover:bg-card/80 transition-colors">
                        <span className="font-bold text-xs sm:text-sm truncate text-right flex-1 min-w-0 pr-1 text-foreground">
                          {schedule.name}
                        </span>
                        <div className="flex items-center gap-1 shrink-0">
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => handleOpenEditDialog(schedule)} title="تعديل الجدول">
                            <SlidersHorizontal className="h-4 w-4" />
                          </Button>
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive/70 hover:text-destructive hover:bg-destructive/10" disabled={schedules.length <= 1} title="حذف الجدول">
            ...
            <SheetFooter className="border-t pt-3 px-1 mt-auto">
              <Button onClick={() => onOpenChange(false)} className="w-full font-bold h-10 rounded-xl shadow-sm">تم</Button>
            </SheetFooter>
          </>
        )}
      </SheetContent>
